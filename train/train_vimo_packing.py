...
class RatioDataset(Dataset):
    """
    Ratio-sample/repeat the base dataset, with multiprocess-synchronized refresh for persistent workers.
    """
...
def sanitize_sample_inplace(x: dict):
    ...
    1. remove all <output_image_x> from messages
...
class SftCollator:
    ...
    def _slice_pixel_values_by_image_indices(self, pixel_values, grid_thw, keep_image_indices):
        ...
        # how many visual tokens each image takes
        per_image_lens = []
        for i in range(grid_thw_cpu.shape[0]):
            t, h, w = grid_thw_cpu[i].tolist()
            per_image_lens.append(int(t * h * w))

        # prefix sum to locate each image's start/end within pixel_values
        offsets = [0]
    ...
    def _truncate_segments_right(self, segments, max_len: int):
        """
        Right-truncate: keep the leftmost max_len tokens.
        Rules:
        - atomic segments (image blocks, prefix, suffix) cannot be cut in half: keep them whole or drop them entirely
        ...
        for seg in segments:
            ...
            # only text may be partially truncated
            kept.append({
                **seg,
                "ids": seg["ids"][:remain],
        ...
        try:
            ...
            for x in batch:
                if getattr(self.args, "use_json_num_tokens", False):
                    ...
                    # images present but num_tokens empty -> raise explicitly (avoid a later crash)
            ...
            # global image-keep indices
            global_keep_input_indices = []
            global_keep_output_indices = []

            # num_tokens kept after truncation
            ...
            for sample_idx, x in enumerate(batch):
                ...
                # ------------------------------------------------------
                # build segments first instead of assembling the final input_ids directly
                # ------------------------------------------------------
                for msg in messages:
                    role = msg["role"]
                    content = msg["content"]
                    ...
                    for m in PLACEHOLDER_PATTERN.finditer(content):
                        ...
                        if image_key.startswith("input_image_"):
                            # compute the und token count from the und grid
                            ...
                            if sample_first_image:
                                ...
                                if sample_token_ptr >= len(sample_num_tokens):
                                    raise ValueError(
                                        f"sample {sample_idx} has insufficient num_tokens, need more for {image_key}, current={sample_num_tokens}"
                                    )
                ...
                # ------------------------------------------------------
                # right-truncate: keep the leftmost max_seq_len tokens
                # ------------------------------------------------------
                ...
                # optional: dedup while preserving the original order
