...
class Qwen3VLTextConfig(PretrainedConfig):
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 151936):
            ...
            `inputs_ids` passed when calling [`DeltaVModel`]
...
class DeltaVConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`DeltaVModel`]. It is used to instantiate a
    Qwen3-VL model according to the specified arguments, defining the model architecture. Instantiating a configuration
    with the defaults will yield a similar configuration to that of
    Qwen3-VL-4B-Instruct [Qwen/Qwen3-VL-4B-Instruct](https://huggingface.co/Qwen/Qwen3-VL-4B-Instruct).
    ...
    ```python
    >>> from deltav.modeling_deltav import DeltaVModel
    >>> from deltav.configuration_deltav import DeltaVConfig

    >>> # Initializing a DeltaV style configuration
    >>> configuration = DeltaVConfig()

    >>> # Initializing a model from the DeltaV style configuration
    >>> model = DeltaVModel(configuration)
...
__all__ = ["DeltaVConfig", "Qwen3VLTextConfig"]
