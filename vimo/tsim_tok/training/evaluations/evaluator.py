...
INCEPTION_V3_PATH = "weights/inception/classify_image_graph_def.pb"
...
class Evaluator:
    ...
    def compute_activations(self, batches: Iterable[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for batch in tqdm(batches, mininterval=5):
...
def _download_inception_model():
    ...
    with requests.get(INCEPTION_V3_URL, stream=True) as r:
        ...
        with open(tmp_path, "wb") as f:
            for chunk in tqdm(r.iter_content(chunk_size=8192), mininterval=5,):
                f.write(chunk)
