...
class BatchNormLocal(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pad = (vb - (B % vb)) % vb
        if pad > 0:
            idx = torch.randint(0, B, (pad,), device=x.device)
            x_pad = x[idx].detach()
...
class DinoDisc(nn.Module):
    def __init__(self, dino_ckpt_path='weights/dinov2/dino_deitsmall16_pretrain.pth',
                 device='cuda', ks=9, depth=12, key_depths=(2, 5, 8, 11), norm_type='bn',
    ...
    def reinit(self, dino_ckpt_path='weights/dinov2/dino_deitsmall16_pretrain.pth',
                 device='cuda', ks=9, depth=12, key_depths=(2, 5, 8, 11), norm_type='bn',
                 using_spec_norm=True, norm_eps=1e-6):
        dino_C = self.dino_proxy[0].embed_dim
        heads = nn.ModuleList([
    ...
    def main(self, x_in_pm1, grad_ckpt=False):  # x_in_pm1: image tensor normalized to [-1, 1]
...
if __name__ == '__main__':
    ...
    ckpt = 'weights/dinov2/dino_deitsmall16_pretrain.pth'
    DinoDisc.forward
