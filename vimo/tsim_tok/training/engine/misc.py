...
import torch
import torch.distributed as dist
def concat_all_gather_varlen(tensor: torch.Tensor, dim: int = 0) -> torch.Tensor:
    """
    All-gather tensors with variable length on `dim` (typically dim=0).
    Pads to max length, all_gather, then trims and concatenates.

    Assumes: all ranks have same tensor rank (ndim) and same sizes except `dim`.
    """
    ...
    lens = torch.stack(lens).view(-1)  # [world_size]
    max_len = int(lens.max().item())

    # 2) pad to max_len on `dim`
    if tensor.size(dim) < max_len:
        ...
        tensor_padded = torch.cat([tensor, pad], dim=dim)
    else:
    ...
    # 4) trim each to its original length and concat
    trimmed = []
    for g, ln in zip(gather_list, lens.tolist()):
        if ln == max_len:
            trimmed.append(g)
        else:
            # slice on dim
            slc = [slice(None)] * g.ndim
            slc[dim] = slice(0, ln)
            trimmed.append(g[tuple(slc)])
    return torch.cat(trimmed, dim=dim)


def concat_all_gather_variable_1d(tensor: torch.Tensor, pad_value: int = -1):
    """
    All-gather for variable-length 1D tensors across ranks.
    Returns concatenated unpadded tensor.
    Note: no gradient, consistent with the original helper.
    """
    if not is_dist_avail_and_initialized():
        return tensor.detach()

    # ensure 1D
    t = tensor.detach().contiguous().view(-1)
    device = t.device
    world = dist.get_world_size()

    # 1) gather lengths
    local_len = torch.tensor([t.numel()], device=device, dtype=torch.long)
    lens = [torch.zeros_like(local_len) for _ in range(world)]
    dist.all_gather(lens, local_len)
    lens = torch.cat(lens, dim=0)  # [world]
    max_len = int(lens.max().item())

    # 2) pad to max_len
    if t.numel() < max_len:
        pad = torch.full((max_len - t.numel(),), pad_value, device=device, dtype=t.dtype)
        t_pad = torch.cat([t, pad], dim=0)
    else:
        t_pad = t

    # 3) all_gather padded
    gathered = [torch.empty((max_len,), device=device, dtype=t.dtype) for _ in range(world)]
