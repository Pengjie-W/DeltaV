...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # ====== delta cross_attn: interleave cross_attn + norm1_delta one step at a time.
        # ZeRO-3 requires every rank to all-gather the same parameters in the same order.
        # The original real path did "batched cross_attn x N, then batched norm1_delta x N",
        # while the padding path interleaved "cross_attn + norm1_delta" pair by pair; the
        # mismatched orders trigger a collective shape mismatch. Making the real path also
        # interleave 1-by-1 keeps the two aligned. ======
        _max_n_delta = getattr(self, '_zero3_max_n_delta', len(src_delta_list))
        local_n_delta = len(src_delta_list)
        _device = src.device

        new_src_delta_list: List[torch.Tensor] = []
        for i in range(_max_n_delta):
            if i < local_n_delta:
                q = src_delta_list[i].unsqueeze(0)
                p = pos_delta_list[i].unsqueeze(0)
                L = q.shape[1]
                ref_1 = self._get_ref_points_for_L(
                    L, device=_device, dtype=torch.float32,
                    num_feature_levels=num_feature_levels,
                )
                ref = ref_1.expand(1, -1, -1, -1).contiguous()
                kv_i = kv_feature_delta[i].unsqueeze(0)
                with torch.autocast('cuda', dtype=torch.float32, enabled=True):
                    q2 = self.cross_attn(
                        ...
                        ref,
                        kv_i,
                        input_spatial_shape_delta,
                        level_start_index_delta,
                    )
                s2 = q2.squeeze(0)
                s = self.norm1_delta(src_delta_list[i] + self.dropout1(s2))
                new_src_delta_list.append(s)
            else:
                with torch.autocast('cuda', dtype=torch.float32, enabled=True):
                    _dum = self.cross_attn(
                        self.with_pos_embed(src[:1], pos[:1]), reference_points[:1],
                        kv_feature[:1], input_spatial_shape, level_start_index)
                # ZeRO-3 modsym: real branch above invokes ``self.dropout1`` (inside
                # the norm1_delta argument). Without this matching call, dummy
                # iterations register one fewer ds_id than real ones, producing
                # ``disagreement between rank0 and rankN`` when ranks have a
                # different real/dummy split of ``_max_n_delta`` iterations.
                _d = self.norm1_delta(src[0, :1, :] + self.dropout1(src[0, :1, :]))
        ...
        _max_n_delta2 = getattr(self, '_zero3_max_n_delta', len(src_delta_list))
        local_n_delta2 = len(src_delta_list)
        if local_n_delta2 > 0:
            new_src_delta_list = []
            for s, a in zip(src_delta_list, tgt2_delta_list):
                ...
                new_src_delta_list.append(s)
            src_delta_list = new_src_delta_list
        # ZeRO-3 modsym: pad dummy norm3_delta + forward_ffn calls.
        # Real iterations above invoke ``self.dropout4(a)`` inside the
        # norm3_delta argument, so each dummy iteration must also call
        # dropout4 to keep the visited submodule sequence symmetric across
        # ranks (previously the dummy was missing one ds_id).
        for _ in range(_max_n_delta2 - local_n_delta2):
            _d = self.norm3_delta(src[0, :1, :] + self.dropout4(src[0, :1, :]))
            _d = self.forward_ffn(_d)
...
class Decoder_Transformer(nn.Module):
    def __init__(self, layer_type, dim, n_base, depth,
                 num_head, mlp_dim, dim_head=64, dropout=0., num_register_tokens=4, image_size=256, decoder_norm=False, rope_1d=False, proj_out_dim=None):
        ...
        self.proj = nn.Linear(dim, proj_out_dim if proj_out_dim is not None else 1024)
