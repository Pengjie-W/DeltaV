...
class Qwen3VLTextConfig(PretrainedConfig):
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 151936):
            ...
            `inputs_ids` passed when calling [`ViMoModel`]
...
class ViMoConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`ViMoModel`]. It is used to instantiate a
    Qwen3-VL model according to the specified arguments, defining the model architecture. Instantiating a configuration
    with the defaults will yield a similar configuration to that of
    Qwen3-VL-4B-Instruct [Qwen/Qwen3-VL-4B-Instruct](https://huggingface.co/Qwen/Qwen3-VL-4B-Instruct).
    ...
    ```python
    >>> from vimo.modeling_vimo import ViMoModel
    >>> from vimo.configuration_vimo import ViMoConfig

    >>> # Initializing a ViMo style configuration
    >>> configuration = ViMoConfig()

    >>> # Initializing a model from the ViMo style configuration
    >>> model = ViMoModel(configuration)
...
__all__ = ["ViMoConfig", "Qwen3VLTextConfig"]
