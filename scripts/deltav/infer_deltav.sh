#!/usr/bin/env bash
# DeltaV interleaved inference (switch the input dataset with JSON_PATH).
...
MODEL_PATH=${MODEL_PATH:-weights/deltav_2b}
EXTRA_CFG=${EXTRA_CFG:-configs/tsim_tok_cfg.json}
JSON_PATH=${JSON_PATH:-data/zebra_infer_sample.json}   # or data/struct_infer_sample.json
EXP_NAME=${EXP_NAME:-deltav_infer}
