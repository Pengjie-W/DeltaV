#!/usr/bin/env bash
# ViMo NORMAL training with SEQUENCE PACKING (DeepSpeed ZeRO-2, no gradient checkpointing).
# Uses the basic ViMo model (vimo/modeling_vimo.py) via entry point train/train_vimo_packing.py.
# This is the lightweight packing recipe; for ZeRO-3 + gradient checkpointing on top of packing
# see scripts/train_vimo_zero3_gc_packing.sh. Packing needs a precomputed per-sample token_length:
...
# See docs/packing.md for details.
set -e
...
TRAIN_SCRIPT=train/train_vimo_packing.py
...
OUTPUT_DIR=${OUTPUT_DIR:-./Checkpoints_MLLM/vimo_packing/}
