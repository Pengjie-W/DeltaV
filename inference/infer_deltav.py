...
from deltav.rope2d import get_rope_index_3
...
EXTRA_CFG = "configs/tsim_tok_cfg.json"
...
def load_model(model_path, extra_cfg_path):
    ...
    vl_chat_processor = DeltaVProcessor.from_pretrained(model_path)
    ...
    vl_gpt = DeltaVModel.from_pretrained(
        model_path,
        config=config,
...
def run_inference(rank, world_size, local_rank, args, dataset):
    ...
    if getattr(args, 'use_tsim_router', False):
        from deltav.tsim_tok.tsim_router import TSIMRouter
